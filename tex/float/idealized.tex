...
\begin{defn}
  We have a \define{Strict Order of Floats} $<$ on idealized floating-point numbers defined
  by
  \begin{enumerate}
    ...
    \item For any nonzreo $m$, $m'$ and any $e\in\ZZ$, we have $m<m'$
      implies $(0,m,e) < (0,m',e)$ and $(1,m,e) > (1,m',e)$.
  \end{enumerate}
  Further, we have an \define{Equality of Floats}
...
\begin{defn}
  \define{Round-Up} is a rounding scheme such that, for any $x\in\RR$,
...
\begin{defn}
  \define{Round-Down} is a rounding scheme such that, for any $x\in\RR$,
...
\begin{thm}[Round-off error bounds]\label{thm:idealized:round-off-error-bounds}
...
\define{Machine Epsilon} to be the least upper bound of relative-error from
